...
| 4 | Как устроена защита | bento | reveal | ink | Controlled prompts · independent judges · instruction hierarchy · suspicious-as-signal · human visibility |
...
> **угроза (2) → принцип (3) → механика (4) → слои (5) → доказательство (6) →
...
> 2. Атаки → #6 horizontal gallery (`seg-lane`/`po-jury`), `injection-attacks` (scroll-snap карточки).
...
- [ ] есть тёмный full-bleed statement (секция 6, honest claim), светлый statement (3),
      pinned-multi-screen слоёв (5), горизонтальная галерея атак (2)
