...
- **✅ Расхождение target↔код — закрыто (фаундер, 2026-06-17).** Ранжирование по
  **human** Jury/Final Score — целевая модель (`human-in-the-loop.md` §13). В коде
  `final_score` был AI-derived (`pipeline-leaderboard.md` §11, `pipeline-scoring.md`
  §9), но human-ранг доделывается **на этой неделе** и будет на сайте — копия «ranks
  on your decision» оставлена как есть. Перед деплоем страницы убедиться, что фича
  выехала в прод.
...
- **Trust-число «1,000+ runs»** — со слов фаундера (2026-06-17). В `ai-jury-prod`
  ...
  пока «400+ runs» — доки отстают. Перед публикацией стоит обновить число в
  источнике правды и на остальных страницах, иначе по сайту разнобой 400+ / 1,000+.
