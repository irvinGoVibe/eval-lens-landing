...
Описывает, как Claude Code агенты в `.claude/agents/` совмещаются со slash-командами `.claude/commands/` для контролируемого цикла разработки EvalLens Landing: от сырого сигнала до финального отчёта.
