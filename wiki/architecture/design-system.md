...
- **Дизайн-система (переиспользуемый слой):** канонический светлый язык —
  scope **`.ds`** в конце `globals.css` (токены `--dsc-*` + lens-soft карточки +
  cool-mist атмосфера + deep media-frame), всё на `:not(.ink)`. Публичные
  компоненты — баррель [web/src/components/ds/index.ts](web/src/components/ds/index.ts):
  `StatementHero · Bento · FullStatement · Gallery · PinnedSteps · Eyebrow ·
  Title · Media · Button`. Подробности — §«Reusable components». **Основные
  страницы дизайн-системы:** `/dev/ds-sections` (референс — собрана из барреля),
  `/dev/ds-atoms` (атомы на light/ink), `/dev/ds-theme` (цельная страница
  light↔ink). На странице: `<main className="section-lab ds">`.
