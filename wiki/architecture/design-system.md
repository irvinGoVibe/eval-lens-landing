...
Дизайн-система EvalLense — единый язык интерфейса. Источник истины — стайл-гайд
самого продукта (`EvalLense — Interface Style Guide`, v1.2), который догфудит
живую систему приложения. **Внутренние страницы сайта строятся на этих же
токенах**, чтобы лендинг и продукт читались как одно целое.
...
| `--ink` | `#0a0a0d` | тёмные панели / statement-поверхности |
| `--panel` | `#121218` | тёмная панель (на 1 ступень светлее ink) |
| `--fg` | `#1d1d1f` | основной текст |
| `--muted` | `#6e6e73` | вторичный текст |
| `--muted-2` | `#86868b` | подписи, mono-лейблы |
...
| `--violet-2` | `#7b5cf6` | вариант фиолетового |
| `--lavender` | `#a99bff` | средняя точка градиента |
...
| Градиент | Параметры | Применение |
|---|---|---|
| Lens — primary | 118°, violet→lavender→cyan→aqua | оценки, знак, ключевые метрики |
| Lens — vivid | 118°, насыщенный | hero-числа, маркетинг |
| Lens — deep | 135°, `#3A1D8F→#1D8F7E` | тёмные поверхности, low-glare |
| Ink — mono | 160°, `#0A0A0D→#1B1B26` | тёмные панели |
| Warm — transition | 118°, amber→orange→red | **только переходы между секциями** |
| Neutral chrome | 180°, `#FFF→#FCFCFD` | chrome окна, тихие бары |
...
Границы: `rgba(255,255,255,.07)` по умолчанию · `.12` акцент. Семантика
десатурируется: green → `#5FE0BF`, amber → `#F0A95A`. Кнопки на тёмном:
...
- Токены: [web/src/app/globals.css](web/src/app/globals.css) — `:root`
  ...
  использовать и Tailwind-утилиты, и legacy-классы.
...
- Стайл-гайд — отдельный HTML вне репозитория (Google Drive). Стоит решить,
  держать ли его копию/экспорт в `wiki/` или ссылаться на внешний источник.
- Полного маппинга всех компонентов стайл-гайда на React-компоненты в `web/`
  пока нет — внутренние страницы создаются по мере необходимости.
