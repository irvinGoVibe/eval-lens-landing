...
| 4   | [Editorial split](#4-editorial-split)                          | `soft` / `ink`         | `reveal left` / `reveal right` | `po-why`, `ab-problem`            | текст + визуал/тезис; самый частый               |
...
| 16  | [FAQ list](#16-faq-list)                                       | `soft`                 | `reveal up`                    | `pr-faq`                          | статичный список Q/A                             |
...
- **Класс:** `eh-problem`, `evr-why`, `hub-statement`, `methodology-rubric`, `injection-boundary`, `usecases-pattern`, `ab-story-claim`, `rb-trail` (светлый вариант)
...
- **Класс:** `po-cta`, `*-cta` на всех страницах; `ct-social` (соцсети)
- **Scroll-FX:** `data-reveal="up"` (стаггер)
...
## Запланированные (ещё не свёрстаны)

Эта таблица — только **свёрстанное**. Целевые приёмы, которых на сайте пока нет,
живут в предписывающем каталоге, не здесь — чтобы инвентарь оставался честным.

- **Type-masked video hero (scroll-scaled)** 🔜 — полноэкранная видео-заставка, по
  скроллу наезжает чёрное поле, заголовок вырезан маской (видео сквозь буквы) и
  уезжает с гигантского размера до нормального. Как
  [apple.com/macbook-pro](https://www.apple.com/macbook-pro/). Описание и как
  собирать — [[page-design-patterns#Каталог блок-архетипов|архетип №10]]. Когда
  свёрстаем — завести строку в таблице выше + скриншот.
