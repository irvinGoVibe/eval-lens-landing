...
| `tr-tone-flip` | transition (L14) | scroll-**pinned** тональный кроссфейд-зона: ink-слой проявляется по ScrollFX `--pin`; опц. `--bloom` = brand mid-frame (никогда не серый); mobile/reduced → static bridge. Вариант `--bloom` | light/soft→ink | ready |
...
> **Sibling (ready):** `tr-tone-flip` (L14, выше в таблице) — это **in-flow** версия того же
> тонального флипа: не сквозной фон всей группы, а одна закреплённая зона **между двумя
> секциями** (свой 150vh, ScrollFX `--pin`, `--bloom`-вариант). Не требует прозрачных секций,
> поэтому НЕ `blocked-by-surface-ownership` и зарегистрирована `ready`. GSAP-flyer choreography
> исходного спайка (летящий заголовок) остаётся page-local (`tone-flip` в `transitions.json`).
...
> 9 transitions (L14, +`tr-overlap-bridge` blocked, +`tr-tone-flip` ready 2026-06-26) · 6 motion recipes (L12:
