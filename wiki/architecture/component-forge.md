...
- **5 Frontend Implementation** — **один** frontend-writer реализует утверждённые
  брифы + медиа в **чистый DS-компонент** (`web/src/components/ds`, баррель
  `@/components/ds`, prefix-free имя, стили в `ds.css`/scope `.ds`) — **не**
  оборачивая и **не** копируя (deprecated) `Lab*`, componentize по карте.
  Контент/CTA/функц.контракт не трогает.
...
(`kb/composition-layers.md`): токены → **DS-атомы** → **DS-каркасы** → **DS-секции**
→ страницы, всё в чистом `@/components/ds` + `ds.css`/`.ds`. (Deprecated субстрат
этих слоёв — `sections/lab/_kit.tsx` / `_layout.tsx` / `Lab*` — постепенно
расковывается в чистый DS; см. [[design-system]] §канон.) Смешанный блок страницы
собирается из DS-атомов+каркасов напрямую, не форкая секцию.
...
- [[component-forge-batch|Component Forge Batch]] — автономный batch-оркестратор над форжем (пачка архетипов одной командой).
