...
| 4A Art Direction | дизайн V1/V2/V3 | `ui-ux-designer` (ритм/рекомпозиция) + `ui-designer` (визуал) |
...
→ 4A Art Direction (V1/V2/V3) → 4B Demo Media → ⛔ Gate A (user)
...
- **4A Art Direction** — три версии одного блока: **V1 Polish** (довести исходную
  ...
  типографика/сцены/scroll-driven). + Designer Self-Check + media-brief.
...
designing → media-curating → design-review → design-approved → implementing →
validating → fixing → final-approved → integrating → completed`; ветка `blocked`
(`design-conflict` / `asset-gap` / `blocked-after-three-fix-cycles`).
