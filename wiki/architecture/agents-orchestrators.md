...
- **Dispatch report.** `/dispatch report` («диспатч репорт») — отчёт по ВСЕМ
  задачам: **отданный воркеру промпт** + **возвращённый ответ** (verbatim). Под это
  по каждой задаче ведётся лог `.claude/runs/dispatch/tasks/<id>.md` (промпт пишется
  при диспетче, return — на завершении). Фильтры: `report #N` / `report done|failed`
  / `report short`. Режим сессии отчёт не меняет.
