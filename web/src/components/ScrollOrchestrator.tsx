...
function runScript() {
  ...
  /* ============================================================
     Generic, page-agnostic scroll FX for internal pages.
     Opt-in purely via data-attributes — keeps every internal page's
     motion inside this ONE shared rAF loop (CLAUDE.md rule: no
     per-section useEffect). The home page uses none of these
     attributes, so this block is a no-op there.

       [data-reveal]                fade/translate in on enter (IO).
         variants: data-reveal="fade|up|left|right|scale"
         stagger:  style="--reveal-delay:120ms"
       [data-scrub]                 writes --scrub 0→1 as the element
         crosses the viewport (parallax, ring fills, scrubbed media).
       [data-pin][data-pin-steps=N] tall track with a sticky inner
         [data-pin-stage]; writes --pin 0→1 across the track and lights
         [data-pin-step] children sequentially (.is-active cumulative,
         .is-current = exactly the active one, --pin-step = its index).

     Respects prefers-reduced-motion: reveals show immediately, pins
     settle fully revealed, scrubs pin to a static end state.
     ============================================================ */
  (function initGenericScrollFX() {
    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    ...
    function paintScrub(el: HTMLElement) {
    ...
    function paintPin(section: HTMLElement) {
