 ...
 * styles), fixed-px type for predictable rendering. Lightly interactive: pick a
 * startup to update the detail panel and toggle the shortlist; switch the bottom
 * view between Leaderboard / Compare / Shortlist. Bar widths are LITERAL
 * Tailwind classes (runtime-built arbitrary values aren't compiled), so they
 * live on the data.
 */
...
function MiniBar({ widthClass, tone }: { widthClass: string; tone: "ai" | "jury" }) {
  const fill = tone === "jury" ? "bg-gradient-to-r from-violet-400 to-cyan-300" : "bg-white/25";
  return (
...
export function ReviewBoardMockup() {
  ...
  return (
    <div className="aspect-[16/9] w-full select-none overflow-hidden rounded-xl border border-white/10 bg-[#080a14] text-white shadow-[0_30px_80px_-40px_rgba(60,40,160,0.6)]">
      <div className="flex h-full flex-col gap-2 p-3 sm:p-4">
        {/* header */}
        <div className="flex shrink-0 items-center justify-between">
          <div className="flex min-w-0 items-center gap-2">
            <span className="font-mono text-[11px] tracking-[0.22em] text-violet-300/90">REVIEW BOARD</span>
            <span className="hidden truncate text-[10px] text-white/35 sm:inline">· Batch ready</span>
          </div>
          <div className="flex items-center gap-1.5 rounded-full bg-white/[0.06] px-2 py-0.5 ring-1 ring-white/10">
            <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-cyan-300" />
            <span className="font-mono text-[10px] text-white/70">24 startups</span>
          </div>
        </div>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[1.7fr_1fr] gap-2">
          ...
          <div className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-lg border border-white/10 bg-white/[0.03]">
            <div className="grid shrink-0 grid-cols-[1fr_auto_2.2rem_2.2rem] items-center gap-2 border-b border-white/10 px-2.5 py-1.5 font-mono text-[9px] uppercase tracking-[0.12em] text-white/35">
              <span>Startup</span>
                ...
                return (
                  <li key={s.name} className="min-h-0 flex-1">
                    <button
                      ...
                      aria-pressed={isSel}
                      className={`group grid h-full w-full grid-cols-[1fr_auto_2.2rem_2.2rem] items-center gap-2 px-2.5 text-left transition-colors duration-200 ${
                    ...
                    >
                      <span className="flex min-w-0 items-center gap-1.5">
                        ...
                        <span className={`truncate text-[12px] ${isSel ? "text-white" : "text-white/80"}`}>
                          {s.name}
                        </span>
                      </span>
                      <span
                        className={`flex items-center gap-1 rounded-full px-1.5 py-0.5 ring-1 ${meta.chip}`}
                      >
                        <span className={`h-1 w-1 rounded-full ${meta.dot} ${meta.pulse ? "animate-pulse" : ""}`} />
                        <span className="whitespace-nowrap text-[9px]">{meta.label}</span>
                      </span>
          ...
          <div className="flex min-h-0 min-w-0 flex-col gap-2 overflow-hidden rounded-lg border border-white/10 bg-white/[0.03] p-2.5">
            <span className="truncate text-[12px] font-medium text-white">{active.name}</span>

            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-md bg-white/[0.03] p-1.5">
                <span className="block font-mono text-[8px] uppercase tracking-[0.1em] text-white/35">
                  AI total
                </span>
                <span className="block font-mono text-[15px] tabular-nums text-white/45">{fmt(active.ai)}</span>
                <MiniBar widthClass={active.aiW} tone="ai" />
                <span className="mt-0.5 block text-[8px] text-white/30">reference</span>
              </div>
              <div className="rounded-md bg-violet-400/[0.08] p-1.5 ring-1 ring-violet-400/15">
                <span className="block font-mono text-[8px] uppercase tracking-[0.1em] text-violet-200/60">
                  Jury score
                </span>
                <span className="block font-mono text-[15px] font-semibold tabular-nums text-cyan-200">
                  {fmt(active.jury)}
                </span>
                <MiniBar widthClass={active.juryW} tone="jury" />
                <span className="mt-0.5 block text-[8px] text-violet-200/45">ranking</span>
              </div>
            </div>

            <div className="flex min-h-0 flex-col gap-1 overflow-hidden">
              <span className="font-mono text-[8px] uppercase tracking-[0.12em] text-white/35">Judge findings</span>
              <div className="flex flex-wrap gap-1 overflow-hidden">
                {active.findings.map((f) => (
                  <span
                    key={f.label}
                    className={`truncate rounded-full px-1.5 py-0.5 text-[9px] ring-1 ${TONE_CHIP[f.tone]}`}
                  >
                    {f.label}
                  </span>
                ))}
        ...
        <div className="shrink-0 rounded-lg border border-white/10 bg-white/[0.03] p-2">
            ...
            {TABS.map((t) => (
              <button
                ...
                className={`rounded-full px-2 py-0.5 transition-colors ${
                  tab === t.id ? "bg-white/10 ring-1 ring-white/15" : "hover:bg-white/[0.04]"
                }`}
              >
                <span
            ...
            <ol className="flex flex-col gap-1">
              {ranked.slice(0, 3).map((r, idx) => (
                <li key={r.s.name} className="flex items-center gap-2">
                  <span className="w-3 shrink-0 font-mono text-[9px] text-white/40">{idx + 1}</span>
                  <span className="w-[28%] shrink-0 truncate text-[10px] text-white/80">{r.s.name}</span>
                  <MiniBar widthClass={r.s.juryW} tone="jury" />
                  <span className="w-7 shrink-0 text-right font-mono text-[10px] tabular-nums text-cyan-200">
                    {fmt(r.s.jury)}
                  </span>
                </li>
              ))}
            ...
            <div className="flex flex-col gap-1">
              {[active, ranked[0].s].map((s, idx) => (
                <div key={`${s.name}-${idx}`} className="flex items-center gap-2">
                  <span className="w-[30%] shrink-0 truncate text-[10px] text-white/75">
          ...
          {tab === "shortlist" ? (
            <div className="flex min-h-[2rem] flex-wrap items-center gap-1">
              {shortlist.length === 0 ? (
                <span className="text-[10px] text-white/35">Star a startup to shortlist it.</span>
              ) : (
                shortlist.map((i) => (
                  <span
                    key={STARTUPS[i].name}
                    className="flex items-center gap-1 rounded-full bg-amber-400/10 px-2 py-0.5 text-[10px] text-amber-200 ring-1 ring-amber-400/20"
                  >
