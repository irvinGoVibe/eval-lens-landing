...
- **создавать новый registry** — он уже есть в `.Codex/library/component-library/`;
...
**Манифесты:** `.Codex/library/component-library/*.json` + `README.md`.
...
**Дизайн-бандл:** `.Codex/designs/evallense/**` (tokens, guidelines, components,
...
`.Codex/agents/wshobson/` не менять. Агенту не передавать весь чат — только task
...
- Новый файл скилла: `.Codex/skills/visual-layer-forge/SKILL.md` (этот).
