...
- менять application-код `web/src/**` (Lab*, Footer, PageHeader, Newsroom, ui/*);
- извлекать новые atoms, создавать `_layout.tsx`, генерировать backgrounds;
- пересобирать страницы, запускать `component-forge` / `build-pages` / `visual-layer-forge`;
- создавать visual layers / backgrounds / transitions / recipes, извлекать компоненты,
  создавать `_layout.tsx`, редизайнить страницы (это работа forge / visual-layer-forge);
- запускать `cd web && pnpm build` без отдельного разрешения пользователя;
- стартовать dev-сервер самостоятельно (правило AGENTS.md — preview только по
...
подтвердил конкретный docs/code-fix. По умолчанию gap уходит в `component-forge`,
а не правится здесь.
...
**по фактическому коду** и **не выдумывать** отсутствующие компоненты.

| # | Слой | Где искать (подтверждено в репо) |
|---|---|---|
| L1 | Design tokens | `web/src/app/globals.css` (`:root`, `@theme inline`), `.Codex/designs/evallense/`, `wiki/architecture/design-system.md` |
| L2 | Global UI primitives | `web/src/components/ui/` (сейчас только `Button.tsx`), глассы/чипы/карточки в `globals.css` |
| L3 | Section Lab atoms | `web/src/components/sections/lab/_kit.tsx` |
| L4 | Layout primitives | `web/src/components/sections/lab/_layout.tsx` — **сейчас ОТСУТСТВУЕТ** (см. §4) |
| L5 | Forged Lab sections | `web/src/components/sections/lab/Lab*.tsx` (16 шт.) |
...
| L11 | Media primitives | `MediaPlaceholder` (`_kit.tsx`), media-слоты в Lab* |
| L12 | Motion primitives | `ScrollFX.tsx`, `ScrollOrchestrator.tsx`, `data-reveal`/`data-scrub`/`data-pin` |
| L13 | Background primitives | `globals.css` surfaces/gradients/glows, `OrangeGlow.tsx`, `UnicornScene/Stage.tsx`, `MistBridge.tsx` |
| L14 | Transition primitives | `MistBridge.tsx`, shared-layer/scrim паттерны между секциями |
| L15 | Composition recipes | `.Codex/library/component-library/recipes.json` — комбинации `surface + background + transition + motion` (ссылки на ID L13/L14/L12); авторит `visual-layer-forge` |

> Реальные пути имеют приоритет. Если что-то в репо отличается от этого списка —
> использовать фактический путь и пометить расхождение в `conflicts`.

---

## 2. Источники истины (читать, что существует)

**Проектные:** `AGENTS.md`, `PROJECT-ENTRYPOINT.md`, `Home.md`, `wiki/index.md`,
...
**Forge:** `.Codex/skills/component-forge/SKILL.md`, `forge-primitives`,
`forge-craft`, `forge-extract`, `forge-validate`, `forge-index`.

**Page building:** `.Codex/skills/build-pages/SKILL.md`.
...
**Стили/дизайн:** `web/src/app/globals.css`, `.Codex/designs/evallense/**`.
...
### Section Lab
| Component | Source path | Rendered in lab | Versions | Surfaces | Status |

### Shared atoms
| Atom | Source | Props | Reused by |
...
`web/src/components/sections/lab/_layout.tsx` **в репозитории сейчас нет**.
Документация может обещать `Band / Wrap / SplitGrid / BentoGrid / GalleryRail /
PinnedStage`, но в manifest их класть **только если файл и exports реально есть**.

Если документация описывает layout layer, а файла/exports нет:
...
Skill объявляет один из двух режимов компоновки для page-composer:

```text
whole-sections-only   # пока нет подтверждённого layout layer — дефолт
atoms-and-layouts     # только когда _layout.tsx существует и экспортирует каркасы
```

До появления подтверждённого layout layer page-composer **не должен** собирать
смешанные блоки из выдуманных каркасов — только цельные секции.

То же про L3 atoms: `_kit.tsx` сейчас экспортирует ровно
`LabContentMode`, `LabContentSet<T>` (типы), `LabEyebrow`, `LabTitle`,
`MediaPlaceholder`. Не предполагать более широкую атомарную систему — фиксировать
фактические exports.
...
### L3 — Section Lab atoms
Для каждого фактического export из `_kit.tsx`: content role, visual role,
required/optional props, motion support, theme behavior, known consumers.

### L4 — Layout primitives
Только реально существующие exports (см. §4). Иначе `documented-missing`.

### L5 — Forged sections — полный consumption contract
```yaml
component:
...
> export → `component-api-gap`. Page Orchestrator видит только фактические exports/
> slots. Логика вариаций: контент→props · внутренний visual→slot · подача→version ·
> layout/reading/motion→new archetype · уникальная сцена→page-local.
...
## 7. Section Lab как живой каталог

`http://localhost:3005/dev/section-lab` — live visual catalog. Использовать
**уже запущенный** preview или поднять `preview_start name=web` (порт 3005), не
менять порт. Учитывать gotcha: в headless-preview скролл на `/dev/section-lab`
сбрасывается в 0 — mid-page блоки проверять через `preview_inspect` /
...
Скриншоты — в `.Codex/library/component-library/screenshots/` (или существующий
...
component-api-gap       # отличие требует форка JSX → передать в component-forge
```

Новый компонент **не создавать** автоматически — gap уходит в `component-forge`.
...
```
Не смешивать `LabGallery` и newsroom article gallery — это разные contracts.
...
```
Помнить инвариант AGENTS.md: вся скролл/анимация-оркестрация — в одном
...
`.Codex/library/component-library/` (уже существует — обновлять, не пересоздавать):
...
|---|---|---|
| Имя экспорта/компонента | `"LabEditorialSplit"`, `"ArticleCard"`, `"Footer"` | grep по `web/src/**` (`export … <Name>`), затем по реестру `component-library.md` |
...
/ lab-atom / layout / lab-section / production-section / chrome / nav / newsroom /
...
### Section Lab
| Archetype | Component | Readiness | Browser QA | Build | Contract |
...
git status --short
git diff --stat
git diff -- .Codex/skills/component-library-preparer .Codex/skills/forge-index \
  wiki/architecture/component-library.md .Codex/library
```
...
Section Lab проиндексирован · все `Lab*` contracts сформированы · props из
реального кода · versions/surfaces/readiness подтверждены · atoms проиндексированы
· layout layer честно отражён (`documented-missing`, если файла нет) ·
...
Манифест — `.Codex/library/component-library/recipes.json`. Контракт — §5.1.
