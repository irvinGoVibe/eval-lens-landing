...
- вручную править манифесты `.Codex/library/component-library/*.json` (регистрация —
...
Сервер/браузер поднимаем **только в Фазе 9 (Full-page QA)** — по правилу AGENTS.md
...
`.Codex/library/component-library/manifest.json` и работает по фактической
...
`.Codex/agents/wshobson/` не менять. Агенту — только task packet (id цели, контракт,
...
### Ф3 — Library Match
Читать **только манифесты** `.Codex/library/component-library/*.json`
...
Тёмные темы — результат дизайн-совета (см. `.Codex/designs/dark-themes/`). База —
...
- **Nebula Deep** (`recipe: style-dark-nebula`, ID `dark-nebula`) — *акцентная тёмная
  ...
  - **двойной video overlay** (`LabCinemaScrim`): readability-слой (`::before`,
...
`.Codex/designs/dark-themes/design-council.md`. Light Vivid/Cinematic ещё могут быть
...
загрузить resume packet (`.Codex/runs/page-composer/<run-id>/resume-packet.json`),
...
`.Codex/runs/page-composer/<run-id>/` (конвенция как у `component-forge-batch`;
...
`.Codex/designs/dark-themes/design-council.md` (источник истины dark-стилей: токены,
