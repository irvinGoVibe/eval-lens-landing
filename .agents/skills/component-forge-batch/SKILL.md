...
```
1. repository rules (AGENTS.md, рейлы оркестратора)
...
```
.Codex/runs/component-forge-batch/<batch-id>/
...
Child runs живут в `.Codex/runs/component-forge/<run-id>/`. **Batch не дублирует
...
на порту 3005 (порт не менять — AGENTS.md).
...
- найти последний **active** batch (`.Codex/runs/component-forge-batch/`);
...
Один **shared** dev server, порт **3005** (канон — `package.json`/`.Codex/launch.json`;
**порт не менять и динамически не выбирать** — AGENTS.md). Lock `dev-server-3005`.
