...
- создавать horizontal overflow, блокировать pointer events, нарушать text contrast,
  ломать геометрию DS-секций (`@/components/ds`; включая ещё не переехавший
  deprecated `.lab-*`/`section-lab` субстрат под ними);
...
Публичный язык поверхностей — scope `.ds` + `.band`; `ink` — это `.band.ink`:
...
- `compose_mode: whole-sections-only` (нет DS-каркаса `_layout.tsx`, конфликт CL-001)
  — Page Orchestrator сейчас собирает целыми `ready` **DS-секциями** (`@/components/ds`),
  не по частям; это его ограничение, не наше, но помнить при recipe.
...
**Код:** `web/src/app/globals.css`, `web/src/components/ds/ds.css` (публичный DS-CSS),
`ScrollFX.tsx`, `ScrollOrchestrator.tsx`, `web/src/components/ds/**` (публичные
DS-секции/атомы), `web/src/components/sections/**` (вкл. `OrangeGlow.tsx`,
`MistBridge.tsx`, deprecated субстрат `lab/_kit.tsx`), `web/src/app/dev/section-lab/page.tsx`
(deprecated стенд), `web/src/app/**/page.tsx`.
...
декора, сочетаемость с DS-секциями (`@/components/ds`), mobile, reduced-motion. Возвращают конкретные
...
ломать геометрию DS-секций (`@/components/ds`; включая deprecated `.lab-*`/`section-lab`
субстрат под ними), не создавать horizontal overflow, не блокировать pointer,
не нарушать контраст, не создавать отдельный runtime. Новый CSS дизайн-системы — в
`ds.css`/`.ds`; кастомные visual-layer стили — аккуратно в конец `globals.css` под
своей обёрткой.
...
слой в живой каталог `/dev/visual-lab` (создать стенд, если его ещё нет; **deprecated
Section Lab не трогать** — это субстрат-каталог секций, другая ось). Full-анализ
библиотеки не запускать.
...
- Реализация (только в рабочем прогоне, не при создании скилла): reusable-примитивы —
  CSS дизайн-системы в `web/src/components/ds/ds.css`/`.ds`, прочий visual-layer CSS в
  `web/src/app/globals.css` (+ при необходимости SVG/React в существующих папках; не в
  deprecated `lab/_kit`).
...
- Живой каталог — **`/dev/visual-lab`** (новый стенд; deprecated Section Lab остаётся
  субстрат-каталогом секций). Не плодить второй каталог, если `/dev/visual-lab` уже
  создан — расширять его.
