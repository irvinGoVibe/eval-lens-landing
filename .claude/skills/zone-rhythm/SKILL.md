...
### 2.1b ⚠️ Dark-start (dark→light→dark): ТЁМНАЯ БАЗА — СНАРУЖИ зоны (fixed)

**Грабли (исправлено на overview + review-board — НЕ ПОВТОРЯТЬ):** если прогон
**начинается с тёмного** (§1 = ink), тёмную **базу** нельзя класть внутрь `.ds-zone`
как `ds-zone__bg--contained`. `--contained` форсит `position:absolute` высотой во всю
зону (тысячи px) вместо `position:fixed` во вьюпорт → **фон расползается по вертикали**
и ломаются viewport-based анимации блобов/спарков.

Правильно (эталон `web/src/app/product/review-board/page.tsx` и
`trust/prompt-injection-safety`):
- `<main>` несёт доп.класс **`ds-canvas`** (даёт `isolation:isolate`, чтобы `z-index:-1`
  скоупился в main).
- Тёмная **база** — `ds-canvas__bg ds-canvas__bg--lobes-dark ds-zone__bg--on` (БЕЗ
  `--contained`, БЕЗ `ds-zone__bg`) **СНАРУЖИ** `.ds-zone`, первым ребёнком `<main>`
  → `position:fixed`, вьюпорт-размер. Спарки — внутрь неё.
- Внутри `.ds-zone` — **только флип-слои** (`--contained`): `--lobes ds-relight` +
  `ds-flip-bridge`/`__glow` + `--lobes-dark ds-redark`. Светлая база НЕ нужна — старт
  идёт на fixed-тёмной базе.
- Швы: T(ink→light) = `<ZoneToneFlipReverse/>` (проявляет `ds-relight`); T(light→ink
  возврат) = `<ZoneToneFlip targetSelector=".ds-redark"/>` (второй тёмный слой).

```jsx
<main className="... section-lab ds ds-canvas">
  {/* база — fixed, СНАРУЖИ зоны */}
  <div className="ds-canvas__bg ds-canvas__bg--lobes-dark ds-zone__bg--on" aria-hidden="true">
    <span className="ds-canvas__spark ds-canvas__spark--1" /> … --2 --3
  </div>
  <div className="ds-zone">
    <div className="ds-zone__bg ds-zone__bg--contained ds-canvas__bg--lobes ds-relight" aria-hidden="true" />
    <div className="ds-flip-bridge" aria-hidden="true" />
    <div className="ds-flip-bridge__glow" aria-hidden="true" />
    <div className="ds-zone__bg ds-zone__bg--contained ds-canvas__bg--lobes-dark ds-redark" aria-hidden="true">
      <span className="ds-canvas__spark ds-canvas__spark--1" /> … --2 --3
    </div>
    <ZoneBlobs top="<клип>" bottom="<клип>" />
    {/* §1 ink … <ZoneToneFlipReverse/> … §light … <ZoneToneFlip targetSelector=".ds-redark"/> … §ink */}
  </div>
  {/* own-backdrop (CtaBand video / Footer) — снаружи зоны */}
</main>
```

> Стопка 2.1 (light-start, светлая база `--contained` внутри зоны) — для прогонов,
> начинающихся со СВЕТЛОГО. Dark-start → стопка 2.1b. Различай по тону §1.
...
- Все bg-слои частичной зоны несут `ds-zone__bg--contained`. **ИСКЛЮЧЕНИЕ:** при
  dark-start (§1 ink) тёмная БАЗА — `ds-canvas__bg ds-zone__bg--on` СНАРУЖИ зоны
  (fixed, БЕЗ `--contained`), `<main>` несёт `ds-canvas`. Внутрь `--contained`
  кладём только флип-слои. Иначе фон расползается по вертикали (см. 2.1b).
