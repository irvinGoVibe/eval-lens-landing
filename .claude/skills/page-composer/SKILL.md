...
```
Ф0 Resolve → Ф1 Current Page Analysis → Ф2 Content Map → Ф3 Library Match
→ Ф4 Reconstruction Plan → Ф5 Page Composition (+ui-ux-pro-max)
→ Ф6 Visual Composition (+ui-ux-pro-max) → Ф7 Gap Routing
→ ⛔ DESIGN REVIEW GATE (user) — единый план; стоп
→ Ф8 Frontend Implementation → Ф9 Full-page QA (3005, fix loop ≤3)
...
### Ф6 — Visual Composition (соседство/целостность)
Назначает каждой секции surface/background/transition/motion **только из `ready`**
visual-layer библиотеки. Анализирует **каждую соседнюю пару**. Оркестратор зовёт
`ui-ux-pro-max` до реализации.
...
visual_composition:
  ...
  transition_before:         # ready tr | none
  transition_after:
...
Не допускать: одинаковых тяжёлых секций подряд; нескольких high-intensity подряд;
повтора одного background без причины; случайного чередования light/soft/ink;
тёмных секций ради разнообразия; резкого black↔white cut без намерения; нескольких
активных motion подряд; перехода, несовместимого с соседними surfaces; blur/glow/mist,
ухудшающих читаемость; декора, конкурирующего с контентом.
...
Собрать единый план (§8) и **остановиться**. Application code — только после явного
`подтверждаю` / `реализуй` / `продолжай` / `approved`. Состояние → `waiting-for-approval`.
...
### Visual composition
| Section | Surface | Background (ready) | Transition (ready) | Motion (ready) |
...
frontmatter) и попросить выбрать одну (`AskUserQuestion`). **Никакого batch.**
...
blocked-after-three-fix-cycles · blocked-design-review · gap-unapproved`.
