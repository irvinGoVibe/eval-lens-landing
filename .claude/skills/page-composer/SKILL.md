...
примитивы уходят в `visual-layer-forge` (§7.2) до сборки. **Dark-темы на момент записи
не зарегистрированы** (новые токены `fg-secondary #d0ccf4`/`#c8c4f0`, recipes
`style-dark-refined`/`style-dark-nebula`, nebula-bg/floor-glow/seam как `ready`
backgrounds/transitions) → при первом выборе dark-стиля он идёт через `visual-layer-forge`
+ `component-library-preparer` incremental, и только потом выносится как `ready`-вариант.
Стиль локально в обход библиотеки не лепить.
