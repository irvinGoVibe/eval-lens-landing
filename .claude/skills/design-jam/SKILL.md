...
- **Дизайнер A — Skill `baoyu-design`.** Делает self-contained HTML-исследование
  направления (мокап/прототип) **в визуальном языке EvalLens** — привязка к
...
1. **Дизайнер A — Skill `baoyu-design`.** Брифуешь: задача, контракт Ф1, привязка
   к бандлу EvalLens (`.claude/designs/evallense/`), light+dark, ограничения DS.
...
- Не создаёт новую DS/палитру/токены/шрифты (EvalLens DS — source of truth).
