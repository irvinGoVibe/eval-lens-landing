...
- **Различия выражаются пропсами, а не форком файла** — prop-driven Server
  Component в **чистом DS** (`@/components/ds`, scope `.ds`, стили в `ds.css`),
  без `Lab`-префикса / `_kit` (`kb/prop-component`). `Lab*` · `_kit` ·
  `sections/lab/*` — **deprecated внутренний субстрат** (north-star: убрать
  целиком), новый код в него не пишется.
...
боевой TSX. Импорт/состав — **только из `@/components/ds`** (баррель — единственный
публичный API сборки); нужного компонента/атома нет в барреле → это **gap** →
forge извлекает **чистый DS** (в `@/components/ds` + `ds.css`), страница в обход
не лепит `Lab*`/`_kit`:
- **Режим B (инлайн)** — извлечь секцию в prop-driven Server Component **чистого
  DS** в `web/src/components/ds` (баррель `@/components/ds`, prefix-free имя, scope
  `.ds`, стили в `ds.css`; контент пропсами, `surface=light|ink`) — **НЕ** оборачивая
  `Lab*` и **НЕ** копируя `Lab*`/`_kit`. Конвенции извлечения —
  `../redraw-block/references/section-componentization.md`. (`Lab*` · `_kit` ·
  `sections/lab/*` · scope `.section-lab` — deprecated субстрат; north-star — убрать.)
