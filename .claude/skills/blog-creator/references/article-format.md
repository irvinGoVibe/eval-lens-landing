...
- `title`: statement-заголовок, **≤40 символов** — на странице статьи к нему добавляется суффикс « — EvalLens Newsroom» (20 симв.), и SERP-лимит 60 иначе режется.
- `date`: `YYYY-MM-DD`, **отображаемая** дата (по ней сортируется лента). Может быть
  задним числом (из контент-плана) — но **никогда не в будущее** и без
  хронологических ляпов (метрики/фичи, которых на ту дату не было). Подробно —
  [content-plan.md](content-plan.md) §P4.
