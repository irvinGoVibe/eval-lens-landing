...
> которые **переопределяют** дефолтные паттерны ниже:
>
> 1. **Движение — только через `ScrollOrchestrator`** (`web/src/components/ScrollOrchestrator.tsx`,
>    единый rAF-цикл). **Запрещено** дробить скролл/анимации на per-section
>    `useEffect` — а это ровно то, что делают `whileInView` / `useInView` /
>    `AnimatePresence` / layout-анимации Framer Motion. В `web/` их **не использовать**.
> 2. **`framer-motion` / `motion` НЕ в зависимостях.** Стек — `three` +
>    react-three-fiber + кастомный ScrollOrchestrator. Добавление пакета — **только
>    после явного согласования с user** («новые зависимости — только по
>    согласованию»). Менеджер — строго **pnpm**.
> 3. **Секции — Server Components.** motion-компоненты клиентские (`"use client"`),
>    что ломает серверность секций под `web/src/components/sections/`.
>
> **Где применять без конфликта:** как API-референс; в дизайн-исследованиях
> `baoyu-design` (self-contained HTML / прототипы **вне** `web/`); как источник
> spring-пресетов / easing / таймингов, которые переносятся в наш `ScrollOrchestrator`
> руками. Прежде чем использовать в `web/` — **спроси user** и получи решение по п.1–2.
