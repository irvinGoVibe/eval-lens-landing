...
- **Структура секций** — таблица: # · Секция · Архетип (из section-types) ·
  ...
  показываем. Соблюдай ритм из шага 3.
- **Контент по секциям** — реальные EN-тексты (eyebrow / heading / subhead /
  буллеты / подписи), по системе копии: statement-first заголовок, глаголы
  действия, конкретика, human-in-the-loop в каждом утверждении про оценку, без
  AI-штампов. Для ключевых строк (heading/CTA) — можно 2–3 варианта пометкой.
